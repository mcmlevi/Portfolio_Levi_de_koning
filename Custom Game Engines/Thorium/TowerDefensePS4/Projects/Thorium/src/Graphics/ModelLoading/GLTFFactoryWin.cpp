...
namespace Th 
{
	Th::ModelData* Th::GLTFFactoryWin::LoadModel(const std::string& filePath)
    {
        ...
        if (ext.compare("glb") == 0) {
            LOGDEBUG("Loading binary GLTF File");
            // assume binary glTF.
            ret = gltf_ctx.LoadBinaryFromFile(&model, &err, &warn,filePath.c_str());
        }
        else {
            LOGDEBUG("Loading ASCII GLTF File");
            // assume ascii glTF.
        ...
        if (!ret) 
        {
            LOGERROR("Failed to parse GLTF file: %s", filePath.c_str());
        }
        Dump(model);
        return nullptr;
    }

    std::string Th::GLTFFactoryWin::GetFilePathExtension(const std::string& FileName)
    {
